...
describe('Segment Service Testing', () => {
        ...
        },
        {
          ...
          useValue: {
            ...
            createQueryBuilder: jest.fn(() => ({
              ...
              getCount: jest.fn().mockResolvedValue(segmentArr.length),
            })),
          },
        },
        {
  ...
  it('should return all segments with status for enrolling experiments and feature flags', async () => {
    const res = {
      segmentsData: [
        ...
        { id: seg2.id, context: 'add', status: SEGMENT_STATUS.USED, subSegments: seg2.subSegments },
      ],
      ...
      allParentSegments: [
        { status: SEGMENT_STATUS.USED, ...seg1 },
        { status: SEGMENT_STATUS.USED, ...seg2 },
      ],
    };
  ...
  it('should clear existing members with a single delete-by-segmentId when editing', async () => {
    ...
    // single "WHERE segment = :id" per member table (via the repository method) rather than a
    // per-row criteria list, and must not preload the full member list first.
    service.checkIsDuplicateSegmentName = jest.fn().mockResolvedValue(false);
    ...
    expect(indivRepo.deleteIndividualForSegmentById).toHaveBeenCalledWith(
      segVal.id,
      expect.any(Object),
      expect.any(Object)
    );
    expect(groupRepo.deleteGroupForSegmentById).toHaveBeenCalledWith(
      segVal.id,
      expect.any(Object),
      expect.any(Object)
    );
  ...
  it('should find all paginated segments with search string name', async () => {
    const res = [
      {
        segmentsData: segmentArr.map((segment) => {
          return { ...segment, status: SEGMENT_STATUS.USED };
