...
const loops_arg = parseInt(process.argv[2] ?? '1', 10) || 1;

quickTest(loops_arg);

/** main test *******************************************************************************/
async function quickTest(loops = 1) {
  for (let i = 0; i < loops; i++) {
    if (loops > 1) console.log(`\n========== Loop ${i + 1} / ${loops} ==========`);
    const loopUserId = loops > 1 ? 'quicktest_user_' + new Date().getTime() + '_' + i : userId;
    await runOnce(loopUserId);
  }
}

async function runOnce(runUserId: string) {
  const client = new UpgradeClient(runUserId, hostUrl, context, options);
  await doInit(client);
