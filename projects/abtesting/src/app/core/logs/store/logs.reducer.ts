...
export const adapter: EntityAdapter<AuditLogs | ErrorLogs> = createEntityAdapter<
  AuditLogs | ErrorLogs
>();
...
const reducer = createReducer(
  initialState,
  on(
    logsActions.actionGetAllAuditLogs,
    (state) => ({ ...state, isAuditLogLoading: true })
  ),
  on(
  ...
  on(
    logsActions.actionGetAllAuditLogsFailure,
    (state) => ({ ...state, isAuditLogLoading: false })
  ),
  on(
    logsActions.actionGetAllErrorLogs,
    (state) => ({ ...state, isErrorLogLoading: true })
  ),
  on(
  ...
  on(
    logsActions.actionGetAllErrorLogsFailure,
    (state) => ({ ...state, isErrorLogLoading: false })
  ),
);

export function logsReducer(
  state: LogState | undefined,
  action: Action
) {
