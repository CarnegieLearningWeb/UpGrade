...
export default async function OrderedRoundRobinAlgoCheck(): Promise<void> {
  ...
  experimentObject = { ...experimentObject, conditionOrder: CONDITION_ORDER.ORDERED_ROUND_ROBIN };
  ...
  expect(experimentConditionAssignments[0].assignedCondition.length).toEqual(conditions.length);

  experimentConditionAssignments[0].assignedCondition.forEach((condition, index) => {
    expect(condition.conditionCode).toEqual(conditions[index].conditionCode);
  });
}
