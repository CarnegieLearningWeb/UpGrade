...
describe('ErrorRepository Testing', () => {
    ...
    it('should save new audit log', async () => {
        createQueryBuilderStub = sandbox.stub(ErrorRepository.prototype, 
    ...
    it('should throw an error when insert fails', async () => {
        ...
        expect(async() => {await repo.saveRawJson(experiment)}).rejects.toThrow(err);
    ...
    it('should throw an error when get total logs fails', async () => {
        ...
        expect(async() => {await repo.getTotalLogs(SERVER_ERROR.CONDITION_NOT_FOUND)}).rejects.toThrow(err);
    ...
    it('should throw an error when find paginated fails', async () => {
        ...
        expect(async() => {await repo.paginatedFind(3, 0, SERVER_ERROR.ASSIGNMENT_ERROR)}).rejects.toThrow(err);
