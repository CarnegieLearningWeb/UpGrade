...
describe('ExperimentAuditLogRepository Testing', () => {
    ...
    it('should save new audit log without entity manager', async () => {
        createQueryBuilderStub = sandbox.stub(ExperimentAuditLogRepository.prototype, 
            'createQueryBuilder').returns(insertQueryBuilder);
    ...
    it('should throw an error when insert fails', async () => {
        ...
        expect(async() => {await repo.saveRawJson(EXPERIMENT_LOG_TYPE.EXPERIMENT_UPDATED, experiment, new User(), manager)}).rejects.toThrow(err);
    ...
    it('should throw an error when get total logs fails', async () => {
        ...
        expect(async() => {await repo.getTotalLogs(EXPERIMENT_LOG_TYPE.EXPERIMENT_CREATED)}).rejects.toThrow(err);
    ...
    it('should throw an error when find paginated fails', async () => {
        ...
        expect(async() => {await repo.paginatedFind(3, 0, EXPERIMENT_LOG_TYPE.EXPERIMENT_CREATED)}).rejects.toThrow(err);
