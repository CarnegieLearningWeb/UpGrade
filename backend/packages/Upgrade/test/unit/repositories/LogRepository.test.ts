import { Connection, DeleteQueryBuilder, EntityManager, UpdateQueryBuilder, SelectQueryBuilder, ConnectionManager } from "typeorm";
...
describe('LogRepository Testing', () => {
    ...
    it('should throw an error when delete except ids fails', async () => {
        ...
        expect(async() => {await repo.deleteExceptByIds([log.id], manager)}).rejects.toThrow(err);
    ...
    it('should throw an error when delete all fails', async () => {
        ...
        expect(async() => {await repo.deleteExceptByIds([], manager)}).rejects.toThrow(err);
    ...
    it('should update log', async () => {
        createQueryBuilderStub = sandbox.stub(LogRepository.prototype, 
    ...
    it('should throw an error when update log fails', async () => {
        ...
        expect(async() => {await repo.updateLog(log.id, {}, new Date("2019-01-16"))}).rejects.toThrow(err);
    ...
    it('should delete by metric id', async () => {
        createQueryBuilderStub = sandbox.stub(LogRepository.prototype, 
            'createQueryBuilder').returns(deleteQueryBuilder);
        let queryStub = sandbox.stub(QueryRepository.prototype, 
        ...
        selectMock.expects('execute').once().returns(Promise.resolve([log]));
        deleteMock.expects('delete').once().returns(deleteQueryBuilder);
        deleteMock.expects('from').once().returns(deleteQueryBuilder);
        deleteMock.expects('where').once().returns(deleteQueryBuilder);
        deleteMock.expects('execute').once().returns(Promise.resolve(result));
    ...
    it('should throw an error when the metric query fails', async () => {
        ...
        expect(async() => {await repo.deleteByMetricId('metrickey')}).rejects.toThrow(err);
    ...
    it('should get metric uniquifier data', async () => {
        let metricStub = sandbox.stub(MetricRepository.prototype, 
            'createQueryBuilder').returns(selectQueryBuilder);
        const result =  {
                identifiers: [ { id: log.id } ],
                generatedMaps: [
    ...
    it('should throw an error when get metric uniquifier data fails', async () => {
        ...
        expect(async() => {await repo.getMetricUniquifierData(['metrickey'], ['uniquifierkey'], 'uid')}).rejects.toThrow(err);
    ...
    it('should analyse a continuous simple metric sum', async () => {
        let experimentStub = sandbox.stub(ExperimentRepository.prototype, 
            'createQueryBuilder').returns(selectQueryBuilder);
        const result =  {
                identifiers: [ { id: log.id } ],
                generatedMaps: [
    ...
    it('should analyse a categorical repeated metric earliest percentage', async () => {
        ...
        selectMock.expects('addGroupBy').once().returns(selectQueryBuilder);
        selectMock.expects('getRawMany').twice().returns(Promise.resolve([data1, data2]));
        let res = await repo.analysis(q);
        ...
        expect(res).toEqual([{"conditionId": 1, "result": 100}, {"conditionId": 2, "result": 100}])
    });
