...
describe('UserRepository Testing', () => {
    ...
    it('should upsert a new user', async () => {
        createQueryBuilderStub = sandbox.stub(UserRepository.prototype, 
    ...
    it('should throw an error when upsert fails', async () => {
        ...
        expect(async() => {await repo.upsertUser(individual)}).rejects.toThrow(err);
    ...
    it('should update user details', async () => {
        createQueryBuilderStub = sandbox.stub(UserRepository.prototype, 
            'createQueryBuilder').withArgs('user').returns(updateQueryBuilder);
        const result =  {
            identifiers: [ { id: individual.email } ],
            generatedMaps: [
                individual
        ...
        let res = await repo.updateUserDetails(individual.firstName, individual.lastName, individual.email, individual.role);
    ...
    it('should throw an error when update user details fails', async () => {
        ...
        expect(async() => {await repo.updateUserDetails(individual.firstName, individual.lastName, individual.email, individual.role)}).rejects.toThrow(err);
    ...
    it('should throw an error when delete fails', async () => {
        ...
        expect(async() => {await repo.deleteUserByEmail(individual.email)}).rejects.toThrow(err);
