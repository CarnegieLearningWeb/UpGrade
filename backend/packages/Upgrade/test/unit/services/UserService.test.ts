...
describe('User Service Testing', () => {
    ...
    it('should return paginated users with search params', async() => {
        ...
        expect(addSelectSpy).toBeCalledWith("ts_rank_cd(to_tsvector('english',concat_ws(' ', coalesce(users.firstName::TEXT,''))), to_tsquery(:query))", 'rank')
    ...
    it('should return paginated users with sort params', async() => {
        ...
        expect(skipSpy).toBeCalledWith(0)
        expect(takeSpy).toBeCalledWith(0)
        expect(addSelectSpy).toBeCalledWith("ts_rank_cd(to_tsvector('english',concat_ws(' ', coalesce(users.lastName::TEXT,''))), to_tsquery(:query))", 'rank')
    ...
    it('should format search string for email', async() => {
        ...
        await service.findPaginated(0, 0, logger, params)
        expect(addSelectSpy).toBeCalledWith("ts_rank_cd(to_tsvector('english',concat_ws(' ', coalesce(users.email::TEXT,''))), to_tsquery(:query))", 'rank')
    })
    ...
    it('should format search string for role', async() => {
        ...
        await service.findPaginated(0, 0, logger, params)
        expect(addSelectSpy).toBeCalledWith("ts_rank_cd(to_tsvector('english',concat_ws(' ', coalesce(users.role::TEXT,''))), to_tsquery(:query))", 'rank')
    })
