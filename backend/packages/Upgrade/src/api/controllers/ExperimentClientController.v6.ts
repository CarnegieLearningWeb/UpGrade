...
export class ExperimentClientController {
   ...
   *         - Omit both `provideGroupsForSession` and `includeStoredUserGroups` parameters
   *
   *         **Ephemeral Mode** (Session-only groups):
   *         - Uses only the groups provided in the session, ignoring any stored user groups
   *         - Set `includeStoredUserGroups` to `false` and provide `provideGroupsForSession`
   *         - Useful for temporary contexts or when complete group information is provided at runtime
   *
   *         **Merged Mode** (Stored + Session groups):
   *         - Combines stored user groups (if exists) with session groups provided in the request
   *         - Set `includeStoredUserGroups` to `true` and provide `provideGroupsForSession`
