...
export function randomCondition(experiment, userID: string, monitoredDecisionPointLogsLength: number) {
  const randomConditionArray: ExperimentCondition[] = [];
  for (let i = 0; i < 100; i++) {
    const uniqueIdentifier: string = experiment.id + userID + i;
    const randomCondition =
      experiment.conditions[Math.floor(seedrandom(uniqueIdentifier)() * experiment.conditions.length)];
...
export function randomRoundRobinCondition(experiment, userID: string, monitoredDecisionPointLogsLength: number) {
  const randomRoundRobinConditionArray: ExperimentCondition[] = [];
  const totalLoopsInQueue = Math.ceil(100 / experiment.conditions.length);
  for (let i = 0; i < totalLoopsInQueue; i++) {
    const tempConditionArray: ExperimentCondition[] = experiment.conditions;
...
export function orderedRoundRobinCondition(experiment, userID: string, monitoredDecisionPointLogsLength: number) {
  const orderedRoundRobinConditionArray: ExperimentCondition[] = [];
  const tempConditionArray: ExperimentCondition[] = experiment.conditions;
...
export function rotateElements(
  conditionArray: ExperimentCondition[],
