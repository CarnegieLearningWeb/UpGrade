...
import { SegmentsEffects_LEGACY } from './segments.effects._LEGACY';
import {
  Segment_LEGACY,
  SegmentFile_LEGACY,
  SegmentInput_LEGACY,
...
describe('SegmentsEffects_LEGACY', () => {
  let store$: any;
  let actions$: ActionsSubject;
  let segmentsDataService: any;
  let router: any;
  let service: SegmentsEffects_LEGACY;
  ...
  const mockSegmentInput: SegmentInput_LEGACY = {
    createdAt: 'test',
    updatedAt: 'test',
    versionNumber: 0,
    id: 'abc123',
  ...
  const mockSegmentFile: SegmentFile_LEGACY = { fileName: 'test', fileContent: JSON.stringify(mockSegment) };
  ...
  beforeEach(() => {
    ...
    service = new SegmentsEffects_LEGACY(store$, actions$, segmentsDataService, router);
  });
  ...
  describe('fetchSegments$', () => {
    it('should dispatch actionFetchSegmentsSuccess with segments data on API call success', fakeAsync(() => {
      ...
      selectAllSegments_LEGACY.setResult([{ ...mockSegment }]);

      const expectedAction = SegmentsActions.actionFetchSegmentsSuccess_LEGACY({
      ...
      actions$.next(SegmentsActions.actionFetchSegments_LEGACY({}));
    ...
    it('should dispatch actionFetchSegmentsFailure on API call failure', fakeAsync(() => {
      ...
      const expectedAction = SegmentsActions.actionFetchSegmentsFailure_LEGACY();
  ...
  describe('upsertSegment$', () => {
    ...
    it('should call createNewSegment(), router nav away to segments page if CREATE_NEW_SEGMENT and actionUpsertSegmentSuccess', fakeAsync(() => {
      ...
      const expectedAction = SegmentsActions.actionUpsertSegmentSuccess_LEGACY({
        segment: { ...mockSegment },
      });
    ...
    it('should call updateSegment(), no router nav, and call actionUpsertSegmentSuccess if UPDATE_SEGMENT', fakeAsync(() => {
      ...
      actions$.next(
        SegmentsActions.actionUpsertSegment_LEGACY({
          segment: { ...mockSegmentInput },
          actionType: UpsertSegmentType_LEGACY.UPDATE_SEGMENT,
        })
      );
    ...
    it('should dispatch actionUpsertSegmentFailure on API error', fakeAsync(() => {
      ...
      const expectedAction = SegmentsActions.actionUpsertSegmentFailure_LEGACY();
  ...
  describe('exportSegments$', () => {
    it('should do nothing if Segment is id', fakeAsync(() => {
      ...
      actions$.next(SegmentsActions.actionExportSegments_LEGACY({ segmentIds: undefined }));
    ...
    it('should dispatch actionExportSegmentSuccess on success', fakeAsync(() => {
      ...
      const expectedAction = SegmentsActions.actionExportSegmentSuccess_LEGACY();
      ...
      actions$.next(SegmentsActions.actionExportSegments_LEGACY({ segmentIds: [{ ...mockSegment }.id] }));
    ...
    it('should dispatch actionExportSegmentFailure on failure', fakeAsync(() => {
      ...
      const expectedAction = SegmentsActions.actionExportSegmentFailure_LEGACY();
  ...
  describe('deleteSegments$', () => {
    it('should do nothing if Segment is id', fakeAsync(() => {
      ...
      actions$.next(SegmentsActions.actionDeleteSegment_LEGACY({ segmentId: undefined }));
    ...
    it('should dispatch actionDeleteSegmentSuccess and navigate to segments page on success', fakeAsync(() => {
      ...
      const expectedAction = SegmentsActions.actionDeleteSegmentSuccess_LEGACY({
        segment: { ...mockSegment },
      });
      ...
      actions$.next(SegmentsActions.actionDeleteSegment_LEGACY({ segmentId: { ...mockSegment }.id }));
    ...
    it('should dispatch actionDeleteSegmentFailure on failure', fakeAsync(() => {
      ...
      const expectedAction = SegmentsActions.actionDeleteSegmentFailure_LEGACY();
