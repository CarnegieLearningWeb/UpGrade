...
import { FEATURE_FLAG_PARTICIPANT_LIST_KEY, IFeatureFlagFile } from 'upgrade_types';
...
export class FeatureFlagsDataService {
  ...
  validateFeatureFlagList(files: IFeatureFlagFile[], flagId: string, listType: FEATURE_FLAG_PARTICIPANT_LIST_KEY) {
  ...
  importFeatureFlagList(files: IFeatureFlagFile[], flagId: string, listType: FEATURE_FLAG_PARTICIPANT_LIST_KEY) {
    const lists = { files: files, listType: listType, flagId: flagId };
