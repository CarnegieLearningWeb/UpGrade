...
import { IImportFile, ValidatedImportResponse } from 'upgrade_types';
...
export interface ImportServiceAdapter {
  validateFiles(files: IImportFile[], params?: any): Observable<ValidatedImportResponse[]>;
  importFiles(files: IImportFile[], params?: any): Observable<ValidatedImportResponse[]>;
...
export class FeatureFlagImportAdapter implements ImportServiceAdapter {
  ...
  validateFiles(files: IImportFile[]): Observable<ValidatedImportResponse[]> {
    return this.featureFlagsDataService.validateFeatureFlag({ files }) as Observable<ValidatedImportResponse[]>;
  }

  importFiles(files: IImportFile[]): Observable<ValidatedImportResponse[]> {
...
export class ExperimentImportAdapter implements ImportServiceAdapter {
  ...
  validateFiles(files: IImportFile[]): Observable<ValidatedImportResponse[]> {
    return this.experimentDataService.validateExperiment(files) as Observable<ValidatedImportResponse[]>;
  }

  importFiles(files: IImportFile[]): Observable<ValidatedImportResponse[]> {
    return this.experimentDataService.importExperiment(files) as Observable<ValidatedImportResponse[]>;
  }

  getLoadingState(): Observable<boolean> {
    return this.experimentService.isLoadingImportExperiment$;
  }
...
export const EXPERIMENT_IMPORT_SERVICE = new InjectionToken<ImportServiceAdapter>('EXPERIMENT_IMPORT_SERVICE');
...
export function provideImportServiceTypeAdapters(): Provider[] {
  return [
    ...
    { provide: EXPERIMENT_IMPORT_SERVICE, useClass: ExperimentImportAdapter },
