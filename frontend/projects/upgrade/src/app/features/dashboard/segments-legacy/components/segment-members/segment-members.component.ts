...
export class SegmentMembersComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    ...
    if (this.segmentInfo) {
      ...
      this.segmentInfo.individualForSegment.forEach((id) => {
        this.members.push(this.addMembers(MemberTypes_LEGACY.INDIVIDUAL, id.userId));
      });
      ...
      this.segmentInfo.subSegments.forEach((id) => {
        this.members.push(this.addMembers(MemberTypes_LEGACY.SEGMENT, id.name));
      });
    }
  ...
  addCSVMembersToTable(segmentMembers: string): void {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (memberType === 'individual') {
        this.members.push(this.addMembers(MemberTypes_LEGACY.INDIVIDUAL, id));
      } else if (memberType === 'segment') {
        if (this.subSegmentIds.includes(id)) {
          this.members.push(this.addMembers(MemberTypes_LEGACY.SEGMENT, id));
        } else {
          this.members.push(this.addMembers(MemberTypes_LEGACY.SEGMENT, ''));
        }
      } else {
  ...
  validateMembers(members: membersTableRowData_LEGACY[]) {
  ...
  validateMembersCount(members: membersTableRowData_LEGACY[]) {
  ...
  setMemberTypes() {
    ...
    this.segmentMemberTypes.push({ name: MemberTypes_LEGACY.INDIVIDUAL, value: MemberTypes_LEGACY.INDIVIDUAL });
    this.segmentMemberTypes.push({ name: MemberTypes_LEGACY.SEGMENT, value: MemberTypes_LEGACY.SEGMENT });
      ...
      case NewSegmentDialogEvents_LEGACY.SEND_FORM_DATA:
        {
          ...
          if (this.segmentMembersForm.valid && !this.membersCountError && this.membersValid) {
            ...
            this.emitSegmentDialogEvent.emit({
              type: this.segmentInfo ? NewSegmentDialogEvents_LEGACY.UPDATE_SEGMENT : eventType,
              formData: segmentMembersFormData,
              path: NewSegmentPaths_LEGACY.SEGMENT_MEMBERS,
            });
          }
        }
        break;
