...
import { MemberTypes_LEGACY, Segment_LEGACY } from '../../../../../core/segments_LEGACY/store/segments.model._LEGACY';
    ...
    this.segmentSub = this.segmentsService.selectedSegment$
      ...
      .subscribe((segment) => {
        ...
        this.segment.individualForSegment?.forEach((user) => {
          this.members.push({ type: MemberTypes_LEGACY.INDIVIDUAL, id: user.userId });
        });
        ...
        this.segment.subSegments?.forEach((subSegment) => {
          this.members.push({ type: MemberTypes_LEGACY.SEGMENT, id: subSegment.name });
        });
      });
