...
export class ExperimentEndCriteriaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.experimentEndForm.get('endExperimentAutomatically').valueChanges.subscribe(
    ...
    this.experimentEndForm.get('endCondition').valueChanges.subscribe(
      endCondition => {
  ...
  updateEndingCriteria() {
    ...
    if (this.experimentEndForm.valid) {
      ...
      const { endExperimentAutomatically, endCondition, dateOfExperimentEnd, userCount, groupCount } = this.experimentEndForm.value;
